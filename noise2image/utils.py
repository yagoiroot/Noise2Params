"""
noise2image.utils — dataset classes and event-count utilities.

Adapted from the public Noise2Image codebase (Cao et al.).  The active
Noise2Params pipeline primarily uses:

* :class:`EventCountNormalization`
    Defined here and made available as a transform, **but not applied**
    anywhere on the Noise2Params pipeline — we found that at 5 s
    integration times it hurt validation-set performance.  Retained
    for parity with the upstream Noise2Image code.  See
    ``README.md`` § "EventCountNormalization is disabled".

* :func:`data_split`
    Deterministic train/val/test split used by
    :file:`train_synthetic_6.py` when no separate validation dataset is
    given.

The other classes in this module (:class:`EventNoiseCount`,
:class:`EventNoiseCountWrapper`, :class:`EventImagePairDataset`) are
on-the-fly synthetic-event generators inherited from Noise2Image.  They
are NOT used by the Noise2Params pipeline, which generates synthetic
events ahead of time with
:func:`noise2params.noise_image.synthetic_data_base_maker` and then
trains from precomputed arrays via
:mod:`noise2image.precomputed_dataset`.  They are retained for parity
with the upstream Noise2Image code and may be useful to users
comparing against the original Noise2Image workflow.
"""
import os
import numpy as np
import torch
...
class EventNoiseCount(object):
    ...
    def __call__(self, sample):
        ...
        p_pos = (1 - scipy.special.erf((np.exp(eps_pos) - 1) * ((im_arr_gamma + self.illum_offset) * self.num_photon_scalar + self.bias_pr) / np.sqrt(
            2 * (im_arr_gamma + self.illum_offset) * self.num_photon_scalar * (1 + np.exp(2*eps_pos)))))
        p_neg = (1 - scipy.special.erf((np.exp(eps_neg) - 1) * ((im_arr_gamma + self.illum_offset) * self.num_photon_scalar + self.bias_pr) / np.sqrt(
            2 * (im_arr_gamma + self.illum_offset) * self.num_photon_scalar * (1 + np.exp(2*eps_neg)))))

        if self.poission_sample:
            count_pos = self.rng.poisson((p_pos * self.num_time))
            count_neg = self.rng.poisson((p_neg * self.num_time))
        else:
            # count_pos = self.rng.negative_binomial(r_pos, r_pos / (p_pos * self.num_time + r_neg)) #bug in original dist
